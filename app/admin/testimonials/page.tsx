      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-8 mb-8 border-b border-white/10">
        <div>
          ...
          <p className="text-xs text-[#cfc3b3] mt-1">
            Manage parent & student testimonials displayed on the homepage slider.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {testimonials.map((item) => (
            ...
            >
              ...
              {/* Bottom Bar: Reviewer Avatar, Author & Actions */}
              <div className="pt-4 border-t border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {item.image ? (
                    <div className="relative w-10 h-10 rounded-full overflow-hidden border-2 border-[#E8A33D] shrink-0">
                      <Image
                        src={item.image}
                        alt={item.name}
                        fill
                    ...
                    <div className="w-10 h-10 rounded-full bg-[#E8A33D]/20 border border-[#E8A33D]/40 text-[#E8A33D] font-bold text-sm flex items-center justify-center shrink-0">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <div>
                <label className="block font-semibold text-[#E8A33D] mb-1.5">
                  Reviewer Person Photo (Cloudinary)
                </label>
                <div className="flex items-center gap-4">
                  {imagePreview ? (
                    <div className="relative w-14 h-14 rounded-full overflow-hidden border-2 border-[#E8A33D] shrink-0">
                      <Image
                        src={imagePreview}
                        alt="Preview"
                        fill
                    ...
                    <div className="w-14 h-14 rounded-full bg-[#211126] border-2 border-dashed border-white/20 flex items-center justify-center text-[#E8A33D] shrink-0">
                      <User className="w-6 h-6 opacity-60" />
                    </div>
                  )}

                  <label className="px-4 py-2.5 rounded-xl bg-[#211126] border border-white/20 hover:border-[#E8A33D] text-xs font-medium cursor-pointer transition-colors flex items-center gap-2">
                    <Upload className="w-4 h-4 text-[#E8A33D]" />
                    <span>{imagePreview ? "Change Photo" : "Upload Reviewer Photo"}</span>
                    <input
                      type="file"
                      accept="image/*"
