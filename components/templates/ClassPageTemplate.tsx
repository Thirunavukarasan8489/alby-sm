...
export const ClassPageTemplate: React.FC<ClassPageTemplateProps> = ({
  ...
  return (
    <main className="min-h-screen bg-[#F8F3E7] text-[#2B2420]">
      ...
      {/* ---------- PAGE HERO ---------- */}
      <section className="bg-[radial-gradient(ellipse_at_75%_20%,rgba(232,163,61,0.18),transparent_55%),linear-gradient(180deg,#211126_0%,#2c1732_100%)] text-[#F8F3E7] pt-[70px] pb-0 px-6">
        <div className="max-w-[1140px] mx-auto grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-12 lg:gap-[50px] items-center">
          ...
          <div className="rounded-[4px] overflow-hidden shadow-[0_30px_60px_-20px_rgba(0,0,0,0.6)] border border-[#E8A33D]/25 self-end mt-5 lg:mt-0">
            <Image
      ...
      <section className="py-[70px] sm:py-[90px] px-6 bg-[#F8F3E7]" id="curriculum">
        ...
        <div className="max-w-[1000px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
          {levelsData.map((lvl) => (
            <div
              key={lvl.number}
              className="bg-white border border-[#17514E]/15 rounded-[5px] p-7 transition-all duration-200 hover:-translate-y-1.25 hover:shadow-[0_18px_30px_-14px_rgba(23,81,78,0.25)]"
            >
              ...
              <ul className="list-none m-0 p-0">
                {lvl.items.map((item, idx) => (
                  <li key={idx} className="text-[13.5px] text-[#5c5147] py-1.25 flex gap-2 items-start">
      ...
      <section className="py-[70px] sm:py-[90px] px-6 bg-[#211126] text-[#F8F3E7]">
        ...
        <div className="max-w-[1140px] mx-auto grid grid-cols-2 md:grid-cols-4 gap-5.5">
          {whyData.map((item, idx) => (
            <div
              key={idx}
              className="bg-[#2c1732] border border-[#E8A33D]/18 rounded-[4px] p-6 text-center"
            >
      ...
      <section className="py-[70px] sm:py-[90px] px-6 bg-[#F8F3E7]">
        ...
        <div className="max-w-[820px] mx-auto overflow-x-auto rounded-[5px] shadow-[0_10px_30px_-18px_rgba(0,0,0,0.2)]">
      ...
      <section className="py-[70px] px-6 bg-[#17514E] text-[#F8F3E7] text-center">
        <blockquote className="font-serif text-xl sm:text-3xl max-w-[720px] mx-auto mb-5 italic leading-[1.4]">
        ...
        <div className="max-w-[1000px] mx-auto grid grid-cols-1 sm:grid-cols-2 gap-6">
          {relatedClasses.map((rel, idx) => (
            <Link
              key={idx}
              href={rel.href}
              className="bg-[#211126] text-[#F8F3E7] rounded-[5px] p-7 flex items-center justify-between transition-transform duration-200 hover:-translate-y-1"
            >
              <div>
                <h3 className="font-serif text-[22px] text-[#F8F3E7]">{rel.title}</h3>
                <span className="text-[13px] text-[#cfc3b3]">{rel.subtitle}</span>
              </div>
              <span className="text-[#E8A33D] text-[22px]">&rarr;</span>
            </Link>
          ))}
        </div>
      </section>
