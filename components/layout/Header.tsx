    ...
    <>
      <header className="sticky top-0 z-50 bg-[#211126]/95 backdrop-blur-md border-b border-white/8">
        <nav className="flex items-center justify-between px-6 py-4 max-w-[1140px] mx-auto">
          ...
          <Link href="/" className="h-14 w-14 rounded-full flex items-center justify-center border-amber/80 border-2">
            <Image
              src={"/logo.png"}
              width={200}
              height={200}
            ...
            {navItems.map((link) => {
              ...
              if (link.hasDropdown) {
                return (
                  ...
                  >
                    <Link
                      href={link.href}
                      className={`text-[14.5px] font-medium transition-all px-3 py-1.5 rounded-[4px] inline-flex items-center gap-1.5 relative ${isActive
                    ...
                    >
                      <span>{link.name}</span>
                      <svg
                        className={`w-3.5 h-3.5 transition-transform duration-200 ${classesDropdownOpen
                          ...
                          {link.subItems?.map((sub) => {
                            ...
                            return (
                              <Link
                                key={sub.href}
                                href={sub.href}
                                className={`flex items-start gap-3 p-2.5 rounded-md transition-all ${isSubActive
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <span
                                    className={`block text-[11.5px] mt-0.5 leading-tight ${isSubActive
                                      ? "text-[#211126]/80"
                                      : "text-[#cfc3b3]"
                                      }`}
                                  >
              ...
              return (
                <li key={link.href} className="relative py-2">
                  <Link
                    href={link.href}
                    className={`text-[14.5px] font-medium transition-all px-3 py-1.5 rounded-[4px] relative ${isActive
                      ? "text-[#E8A33D] font-semibold bg-white/6"
                      : "text-[#F8F3E7]/85 hover:text-[#E8A33D] hover:bg-white/4"
                      }`}
                  >
          ...
          >
            <span
              className={`w-6 h-0.5 bg-[#F8F3E7] rounded-full transition-transform duration-300 ${mobileMenuOpen ? "rotate-45 translate-y-2 bg-[#E8A33D]" : ""
                }`}
            />
            <span
              className={`w-6 h-0.5 bg-[#F8F3E7] rounded-full transition-opacity duration-300 ${mobileMenuOpen ? "opacity-0" : "opacity-100"
                }`}
            />
            <span
              className={`w-6 h-0.5 bg-[#F8F3E7] rounded-full transition-transform duration-300 ${mobileMenuOpen ? "-rotate-45 -translate-y-2 bg-[#E8A33D]" : ""
