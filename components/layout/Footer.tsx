import React from "react";
import Link from "next/link";
import { ACADEMY_INFO } from "@/lib/constants";
import { MapPin, Phone, Mail, Clock, ChevronRight } from "lucide-react";

// const InstagramIcon = () => (
//   <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
//     <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
//   </svg>
// );

const InstagramIcon = () => (
  <svg
    className="w-4 h-4 transition-transform duration-200 group-hover:scale-110"
    viewBox="0 0 24 24"
  >
    <radialGradient id="igGrad" cx="30%" cy="107%" r="128%" fx="30%" fy="107%">
      <stop offset="0%" stopColor="#fdf497" />
      <stop offset="5%" stopColor="#fdf497" />
      <stop offset="45%" stopColor="#fd5949" />
      <stop offset="60%" stopColor="#d6249f" />
      <stop offset="90%" stopColor="#285AEB" />
    </radialGradient>
    <path
      fill="url(#igGrad)"
      d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"
    />
  </svg>
);

const YoutubeIcon = () => (
  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
    <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
  </svg>
);

const WhatsappIcon = () => (
  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
    <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z" />
  </svg>
);

export const Footer: React.FC = () => {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="bg-[#1c0d20] text-[#cfc3b3] border-t border-[#E8A33D]/15 mt-auto">
      {/* Main Footer Container */}
      <div className="max-w-7xl mx-auto px-6 pt-12 pb-10 sm:pt-16 sm:pb-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          {/* Column 1: Brand & Bio */}
          <div className="flex flex-col gap-4">
            <Link href="/" className="inline-block">
              <span className="font-serif text-2xl sm:text-3xl text-[#F8F3E7] font-bold tracking-wide">
                Alby{" "}
                <span className="italic text-[#E8A33D] font-normal">
                  School of Music
                </span>
              </span>
            </Link>
            <p className="text-xs sm:text-sm text-[#b7aa9c] leading-relaxed">
              Structured, ear-first music education for Piano, Guitar, and
              Electronic Keyboard in Coimbatore. Guiding students from age 6+ to
              Trinity College London grade exams.
            </p>
            <div className="flex items-center gap-3 pt-2">
              <a
                href={ACADEMY_INFO.socials.instagram}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Instagram"
                className="w-9 h-9 rounded-full bg-[#211126] border border-white/10 flex items-center justify-center text-[#F8F3E7] hover:bg-[#fff] transition-colors"
              >
                <InstagramIcon />
              </a>
              <a
                href={ACADEMY_INFO.socials.youtube}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="YouTube"
                className="w-9 h-9 rounded-full bg-[#211126] border border-white/10 flex items-center justify-center text-[#f10101] hover:bg-[#fff] hover:text-[#f10101] transition-colors"
              >
                <YoutubeIcon />
              </a>
              <a
                href={ACADEMY_INFO.whatsappUrl}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="WhatsApp"
                className="w-9 h-9 rounded-full bg-[#211126] border border-white/10 flex items-center justify-center text-[#25D366] hover:bg-[#25D366] hover:text-white transition-colors"
              >
                <WhatsappIcon />
              </a>
            </div>
          </div>

          {/* Column 2: Quick Navigation */}
          <div>
            <h3 className="font-serif text-lg text-[#F8F3E7] font-semibold mb-4 tracking-wide relative inline-block">
              Quick Navigation
              <span className="block h-0.5 w-8 bg-[#E8A33D] mt-1 rounded-full" />
            </h3>
            <ul className="space-y-2.5 text-xs sm:text-sm">
              {[
                { label: "Home", href: "/" },
                { label: "About Us & Faculty", href: "/about" },
                { label: "All Classes Overview", href: "/classes" },
                { label: "Recital & Performance Gallery", href: "/gallery" },
                { label: "Book Your Slots", href: "/contact" },
                { label: "Privacy Policy", href: "/privacy-policy" },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="group inline-flex items-center gap-1.5 text-[#cfc3b3] hover:text-[#E8A33D] transition-colors"
                  >
                    <ChevronRight className="w-3.5 h-3.5 text-[#E8A33D]/60 group-hover:translate-x-0.5 transition-transform" />
                    <span>{link.label}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 3: Music Programs */}
          <div>
            <h3 className="font-serif text-lg text-[#F8F3E7] font-semibold mb-4 tracking-wide relative inline-block">
              Music Programs
              <span className="block h-0.5 w-8 bg-[#E8A33D] mt-1 rounded-full" />
            </h3>
            <ul className="space-y-2.5 text-xs sm:text-sm">
              {[
                {
                  label: "Piano Class (Classical & Contemporary)",
                  href: "/classes/piano",
                },
                {
                  label: "Guitar Class (Acoustic & Electric)",
                  href: "/classes/guitar",
                },
                {
                  label: "Electronic Keyboard Class",
                  href: "/classes/keyboard",
                },
                { label: "Trinity Exam Prep (Grades 1-8)", href: "/#exams" },
                { label: "Rockschool RSL Exam Prep", href: "/#exams" },
              ].map((program) => (
                <li key={program.label}>
                  <Link
                    href={program.href}
                    className="group inline-flex items-center gap-1.5 text-[#cfc3b3] hover:text-[#E8A33D] transition-colors"
                  >
                    <ChevronRight className="w-3.5 h-3.5 text-[#E8A33D]/60 group-hover:translate-x-0.5 transition-transform" />
                    <span>{program.label}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 4: Contact & Academy Info */}
          <div>
            <h3 className="font-serif text-lg text-[#F8F3E7] font-semibold mb-4 tracking-wide relative inline-block">
              Contact & Location
              <span className="block h-0.5 w-8 bg-[#E8A33D] mt-1 rounded-full" />
            </h3>
            <ul className="space-y-3 text-xs sm:text-sm text-[#cfc3b3]">
              <li className="flex items-start gap-2.5">
                <MapPin className="w-4 h-4 text-[#E8A33D] shrink-0 mt-0.5" />
                <span>{ACADEMY_INFO.formattedAddress}</span>
              </li>
              <li className="flex items-center gap-2.5">
                <Phone className="w-4 h-4 text-[#E8A33D] shrink-0" />
                <a
                  href={`tel:${ACADEMY_INFO.phoneRaw}`}
                  className="hover:text-[#E8A33D] transition-colors"
                >
                  {ACADEMY_INFO.phone}
                </a>
              </li>
              <li className="flex items-center gap-2.5">
                <Mail className="w-4 h-4 text-[#E8A33D] shrink-0" />
                <a
                  href={`mailto:${ACADEMY_INFO.email}`}
                  className="hover:text-[#E8A33D] transition-colors"
                >
                  {ACADEMY_INFO.email}
                </a>
              </li>
              <li className="flex items-start gap-2.5 pt-1">
                <Clock className="w-4 h-4 text-[#E8A33D] shrink-0 mt-0.5" />
                <span className="text-[11.5px] leading-relaxed">
                  {ACADEMY_INFO.openingHours}
                </span>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom Bar Divider */}
        <div className="mt-12 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-[#a89b8c]">
          <p>
            © {currentYear} {ACADEMY_INFO.name}. All rights reserved.
          </p>
          <div className="flex items-center gap-6">
            <Link
              href="/privacy-policy"
              className="hover:text-[#E8A33D] transition-colors underline decoration-white/20"
            >
              Privacy Policy
            </Link>
            <Link
              href="/contact"
              className="hover:text-[#E8A33D] transition-colors"
            >
              Book Your Slots
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
};
